...
const DownloadButtonField = ({ record }: { record?: any }) => {
    ...
    const handleDownload = async () => {
        ...
        if (!downloadUrl) {
            ...
            setSuccessMessage(null); // if there is error - success message will be null
            return;
        ...
        const fileName = record.fileName || "warranty_file.jpg"; // default name if we couldn't fetch the name 

        console.log(`📂 Saving file as: ${fileName}`);

        const link = document.createElement("a");
        link.href = URL.createObjectURL(blob);
        link.setAttribute("download", fileName); //web browser will use the file name
